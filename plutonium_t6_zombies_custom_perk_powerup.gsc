...
init()
{
	level thread on_random_perk_bonus_drop();
	level thread force_drop_every_kill();
	...
	set_zombie_var( "zombie_powerup_drop_increment", 1 );
	set_zombie_var( "zombie_powerup_drop_max_per_round", 999 );
	...
	iprintlnbold("^2[DEBUG] Perk powerup init() ran");
...
watch_perk_button()
{
    ...
    for(;;)
    {
        ...
        {
            // Drops the custom perk bottle at the player's feet
            maps\mp\zombies\_zm_powerups::specific_powerup_drop("random_perk", self.origin);
            
            // Cooldown to prevent infinite spawning
            wait 2; 
        }
        wait 0.05;
    }
...
on_random_perk_bonus_drop()
{
    ...
    for(;;)
    {
        level waittill("zombie_death", zombie);
		self iprintlnbold("Random perk bonus drop");

        if(randomint(100) < 10) // 10% chance per kill, adjust as needed
            maps\mp\zombies\_zm_powerups::specific_powerup_drop("random_perk", zombie.origin);
    }
}
force_drop_every_kill()
{
    level endon("end_game");
    for(;;)
    {
        level waittill("zombie_death", zombie);
		self iprintlnbold("Drop every kill");
        maps\mp\zombies\_zm_powerups::specific_powerup_drop("random_perk", zombie.origin);
    }
}
wait_end_game()
...
onPlayerConnect()
{
    ...
    for(;;)
    {
		...
		self iprintlnbold("Loaded the script!!!");
...
bert()
{
	...
	for( ;; ){
		...
		if(randomint(100) < 13){
			if(randomint(100) < 90){
				dropRandomPowerup(zomb.origin);
			}
			else{
				specific_powerup_drop("random_perk", zomb.origin);	
			}
		}
	}
}

dropRandomPowerup(drop_origin)
{
    // Define an array of standard power-up names available in BO2 Zombies
    powerups = [];
    powerups[0] = "insta_kill";
    powerups[1] = "full_ammo";
    powerups[2] = "double_points";
    powerups[3] = "carpenter";
    powerups[4] = "nuke";
    powerups[5] = "fire_sale";
    powerups[6] = "bonfire_sale";

    random_index = RandomInt(powerups.size);
    chosen_powerup = powerups[random_index];

    // Drop the power-up at the specified origin (usually a player or zombie position)
    // The second argument is the drop origin, and the third is whether it was spawned via a specific event (false)
    maps\mp\zombies\_zm_powerups::specific_powerup_drop(chosen_powerup, drop_origin);
}
...
PlayerDownedWatcher()
{
    ...
    for(;;)
    {
        // Capture WHICH event triggered the continuation
        event = self waittill_any_return( "fake_death", "player_downed", "player_revived", "spawned_player", "disconnect", "death", "end_game" );

        // Only save loadout and spawn the tombstone drop if the player actually went down
        if( event == "player_downed" || event == "fake_death" )
        {
...
{
	self iprintlnbold("Should drop limited");
...
{
	//iprintlnbold("^2[DEBUG] Powerup grabbed: " + s_powerup.powerup_name);
...
custom_give_perk( perk, bought, custom, saved_perk )
{
    ...
    else
    {
        ...
        else
        {
			...
			self start_custom_perk_logic( perk );
            self.num_perks++;
        }
    }
	self iprintln(perk);
...
give_random_perk()
{
	//iprintlnbold("^2[DEBUG] give_random_perk() called");
    perks = array();
    //CUSTOM PERKS
